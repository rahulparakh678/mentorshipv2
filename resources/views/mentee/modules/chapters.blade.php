                ...
                @foreach($chapters as $chapter)
                    <div id="chapter-content-{{ $chapter->id }}" class="chapter-content" style="display: none;">
                        ...
                        <div class="nav-buttons">
                            ...
                            @if($chapter->quizAttempted)
                                <!-- Optionally, you could disable or remove the quiz button -->
                                <a href="#" class="btn btn-secondary disabled">Quiz Attempted</a>
                            @else
                                @if($chapter->has_mcq)
                                    <a href="{{ route('viewquiz', ['chapter_id' => $chapter->id]) }}" class="btn">Start Quiz</a>
                                @else
                                    <a href="{{ route('getDiscussionQuestions', ['chapter_id' => $chapter->id]) }}" class="btn">Discussion Points</a>
                                @endif
                            @endif
                            ...
                            {{--@if($chapter->has_mcq)
                                @if($chapter->quizAttempted)
                                    <button class="btn" disabled>Start Quiz</button>
                                @else
                                    <a href="{{ route('viewquiz', ['chapter_id' => $chapter->id]) }}" class="btn">Start Quiz</a>
                                @endif
                            @else
