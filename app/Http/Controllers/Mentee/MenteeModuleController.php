...
class MenteeModuleController extends Controller
{
    ...
    public function showchapters(Request $request)
    {
        ...
        // Get the max quiz result for the user and module
        ...
        // Map through each chapter and check if the quiz has been attempted
        $chapters->map(function ($chapter) use ($user_id, $module_id, $maxResult) {
            // Check if there is any quiz result for the current module and user
            $quizResult = DB::table('quiz_results')
                ->where('user_id', $user_id)
                ->where('module_id', $module_id)
                ->first();

            // If the quiz result exists, mark quiz as attempted
            $chapter->quizAttempted = $quizResult ? true : false;

            // Optionally, add the score info to the chapter
            if ($quizResult) {
                $chapter->quizScore = $quizResult->score;
            }
